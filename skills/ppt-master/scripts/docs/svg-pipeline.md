...
> This is the **legacy compatibility export** path. Use it only when the user explicitly wants `ppt-master` itself to export PPTX directly, or when the native editable rebuild path is unavailable. For final editable decks, prefer the default skeleton review flow followed by the internal native rebuild phase.
...
- If the user wants a final deck that remains broadly editable, stop after skeleton review and continue to the internal native rebuild phase instead

...
PPT Master is an AI-driven presentation workflow. Through multi-role collaboration (Strategist → Image_Generator → Skeleton Executor), it converts source documents (PDF/DOCX/URL/Markdown) into a reviewable skeleton package first, then routes a confirmed project to one selected final target: offline single-file HTML or native editable `.pptx` production.
...
- ⚠️ **Final-target selection**: Record one selected final target before final production. Do not produce final HTML and PPTX simultaneously by default. `preview/index.html` is the review draft, not the final HTML.
